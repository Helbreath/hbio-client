...
//정진광 수정, constructor OverLoad..
//기존의 constructor 와 구분을 하기 위해, int type variable 을 parameter로 추가했으나 실제로는 안쓰임.
//이 constructor는 내부에서 Pak 파일에 대한 open 과 close 를 하지 않는다.
CSprite::CSprite(HANDLE hPakFile, DXC_ddraw *pDDraw, char *cPakFileName, short sNthFile, bool bAlphaEffect)
{
	...
	//i+100은 ASD파일의 시작에서부터 Sprite Confirm부분을 뛰어넘는다.
	...
	// 알파 에펙트가 사용되는 스프라인트인지 
	m_bAlphaEffect = bAlphaEffect;
	// 스프라이트 정보만을 읽고 이미지는 로딩하지 않는다. 최초로 그릴때 로딩한다. 
}
...
void CSprite::iRestore()
{
 HDC     hDC;
	...
	// 서페이스가 할당되어있지 않다면 Restore할 필요가 없다.
	// 비트맵이 로드되지 않은 스프라이트는 복구할 필요가 없다. 
...
void CSprite::PutSpriteRGB(int sX, int sY, int sFrame, int sRed, int sGreen, int sBlue, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	// 속도향상을 위해 미리 계산해 놓는다. 의미가 있을지는 모르겠음 
		...
		case 1:
			for (iy = 0; iy < m_wBitmapSizeY; iy++) {
				for (ix = 0; ix < m_wBitmapSizeX; ix++) {
					if (pSrc == NULL) return; // v2.1 메모리가 128M 이하인 컴에서는 여기서 자주 에러가 뜬다.
					if (pSrc[ix] != m_wColorKey) {
...
//	else return FALSE;
	...
	pSrc += m_sPitch * ( msY - tY - 3 );//이 코드는 (-2,-2)부터 (2,2) 까지 검색하기 위한거다.
	for( iy=0 ; iy<=6 ; iy++ )
	{
