...
CMapData::CMapData(class CGame * pGame)
{
	...
	for (i = 0; i < DEF_TOTALCHARACTERS; i++ ) {
		m_stFrame[i][DEF_OBJECTMOVE].m_sMaxFrame = 7;//일단 모든 오브젝트의 MOVE프레임은 7로 하고..
	}
	// 캐릭터
	for (i = 1; i <= 6; i++) {
...
BOOL __fastcall CMapData::bSetOwner(WORD wObjectID, int sX, int sY, int sType, int cDir, short sAppr1, short sAppr2, short sAppr3, short sAppr4, int iApprColor, short sStatus, char * pName, short sAction, short sV1, short sV2, short sV3, int iPreLoc, int iFrame)
{
 ...
 int   iEffectType, iEffectFrame, iEffectTotalFrame;
	...
	//////////////// ++++++++++
	// 맵 데이터 구조를 벗어나는 이벤트라면 즉각 맵에서 삭제한다.
	if ((wObjectID < 30000) && ((sX < m_sPivotX) || (sX >= m_sPivotX + MAPDATASIZEX) || 
		...
		} //Close 맵 데이터를 벗어난 이벤트 처리...
		////////////////////////////////////////////
		
		// 위의 처리에서 찾지 못했다면 전체 맵을 검색하여 삭제한다.
		for (iX = 0; iX < MAPDATASIZEX; iX++)
		for (iY = 0; iY < MAPDATASIZEY; iY++) {
	...
	if ((wObjectID < 30000) && (sAction != DEF_OBJECTNULLACTION)) {
		...
		// 아무곳에서도 검색되지 않았다. 최초로 도착한 데이터이므로 좌표만 입력 
		// v1.41 캐쉬 좌표 저장 
		m_iObjectIDcacheLocX[wObjectID] = sX;
		m_iObjectIDcacheLocY[wObjectID] = sY;
	}
		...
		// v1.4 먼저 Cache 좌표로 검색하여 일치하면 처리한다. 
		////////////////////////////////////////////
		if (m_iObjectIDcacheLocX[wObjectID] > 0) { 
			// 양수. 위에 서 있다.
			iX = m_iObjectIDcacheLocX[wObjectID] - m_sPivotX;
			...
			if (m_pData[iX][iY].m_wObjectID == wObjectID) {
				...
				if ((wObjectID != (WORD)m_pGame->m_sPlayerObjectID) && 
					...
					// 새로 할당하고자 하는 곳에 자신이 아닌 다른 오브젝트가 서 있다면 할당할 수 없고, 
					// 이 오브젝트의 현재 위치를 요청한다.
					// 전체 데이터가 없는 오브젝트의 이벤트이다. 서버로 전체 데이터를 요청한다. 
				...
				iChatIndex = m_pData[iX][iY].m_iChatMsg;
				if (sAction != DEF_OBJECTNULLACTION) {
					sType      = m_pData[iX][iY].m_sOwnerType; //v1.12 윗줄에서 포함됨
			...
			if ((m_pData[iX][iY].m_cDeadOwnerFrame == -1) && (m_pData[iX][iY].m_wDeadObjectID == wObjectID)) {
				// 동작에 따른 최종 위치 계산 
				// 이곳에 다른 오브젝트가 이미 있다면 명령은 취소되고 모든 데이터 전송을 요구하게 된다. 
				dX = iX;
				dY = iY;
				switch (sAction) {
				case DEF_OBJECTMOVE:
				...
				iChatIndex = m_pData[iX][iY].m_iDeadChatMsg;
				if (sAction != DEF_OBJECTNULLACTION) {
			   		sType      = m_pData[iX][iY].m_sDeadOwnerType; // v1.12 윗줄에서 포함됨
		...
		// 위의 검색에서 찾지 못했다면 여기서 처리한다.
		// ObjectID 값으로 검색하는 경우. 
		// 같은 ObjectID를 갖는 object를 찾아 이름, 특성치등등을 복사해서 사용한다. 어떤 좌표값도 없다!!!
		
		for (iX = 0; iX < MAPDATASIZEX; iX++)
		for (iY = 0; iY < MAPDATASIZEY; iY++) {
			
			if (m_pData[iX][iY].m_wObjectID == wObjectID) {
				// 인접한 곳에서 같은 ObjectID를 가진 오브젝트를 발견하였다. 
...
int CMapData::iObjectFrameCounter(char * cPlayerName, short sViewPointX, short sViewPointY)
{
 ...
 long  lPan;
	...
	for (dY = 0; dY < MAPDATASIZEY; dY++) {
		...
		if ((dwTime - m_dwDOframeTime) > 100) { //v1.4

			// v1.432 에펙트 프레임 카운트 증가 및 효과별 프레임 루핑 조정
		...
		if ((m_pData[dX][dY].m_cDeadOwnerFrame >= 0) && ((dwTime - m_pData[dX][dY].m_dwDeadOwnerTime) > 150)) {
			...
			if (m_pData[dX][dY].m_cDeadOwnerFrame > 10) {
				// 죽은 캐릭터를 없앤다. 
				m_pData[dX][dY].m_wDeadObjectID  = NULL; // -1; v1.41
		...
		if (m_pData[dX][dY].m_sOwnerType != NULL) {
			...
			case DEF_OBJECTMAGIC:
				if( m_pGame->m_cSkillMastery[4] == 100 ) iDelay = -13;// 마법 능력이 100이면 캐스팅 타임이 약간 빨라진다.
				else iDelay = 0;
				break;
			...
			if ((dwTime - m_pData[dX][dY].m_dwOwnerTime) > dwFrameTime ) {
				...
				if (m_pData[dX][dY].m_cOwnerFrame > m_stFrame[m_pData[dX][dY].m_sOwnerType][m_pData[dX][dY].m_cOwnerAction].m_sMaxFrame) {
					if ((m_sRectX <= dX) && ((m_sRectX + 20) >= dX) &&
						...
						// (!) 만약 사망동작이 끝났으면 DeadOwner로 위치를 바꾸어야 한다. 
						if (m_pData[dX][dY].m_cOwnerAction == DEF_OBJECTDYING) {
							...
							m_pData[dX][dY].m_wObjectID  = NULL; //-1; v1.41
							m_pData[dX][dY].m_sOwnerType = NULL;
						...
						// 카운팅이 끝난 객체가 범위 밖에 있으면 무조건 삭제한다.
				...
				if ( m_pData[dX][dY].m_cOwnerAction == DEF_OBJECTSTOP ) { 
					...
					case 40: // 에너지 실드 제네레이터
					case 41: // 그랜드 매직 제네레이터 
					case 42: // 마나 스톤
						if ((rand() % 40) == 25) {
				...
				if ( (m_pData[dX][dY].m_cOwnerAction == DEF_OBJECTATTACK) ) { 
					// 공격동작인 경우 특정 프레임서 Effect 효과를 생성해야 한다.
					switch (m_pData[dX][dY].m_sOwnerType) {
					case 1:
					case 2:
					case 3:
					...
					case 6:
						...
						if (m_pData[dX][dY].m_cOwnerFrame == 5) {
							// 검광이나 기타 화살발사등의 Effect를 생성한다.
							if ((m_pData[dX][dY].m_sAppr2 & 0xF000) != 0) {
								// 전투 모드일때만 에펙트가 생성된다.
								if (m_pData[dX][dY].m_sV3 != 1) {
																		
									m_pGame->bAddNewEffect(m_pData[dX][dY].m_sV3, m_sPivotX + dX, m_sPivotY + dY, m_sPivotX + dX + m_pData[dX][dY].m_sV1, m_sPivotY + dY + m_pData[dX][dY].m_sV2, 0, m_pData[dX][dY].m_sOwnerType);
									// 필살기 효과음 
					...
					case 6:
						// 전투 모드일때만 효과음이 생성된다.
						if ((m_pData[dX][dY].m_sAppr2 & 0xF000) != 0) {
							// 무장하고 있는 무기의 종류에 따라 적절한 사운드를 발생시킨다. 
							sWeaponType = ((m_pData[dX][dY].m_sAppr2 & 0x0FF0) >> 4);
				...
				if ( m_pData[dX][dY].m_cOwnerAction == DEF_OBJECTMAGIC ) { 
					...
					case 6:
						if (m_pData[dX][dY].m_cOwnerFrame == 1) {
							...
							// v1.5 m_pData[dX][dY].m_sV2가 마법 시전자의 마법 능력. 여기에 맞는 효과를 생성한다.
...
//								m_pGame->bAddNewEffect(74, (m_sPivotX+dX)*32, (m_sPivotY+dY)*32, 0, 0, 0 );
	...
	// 화면상에 움직이는 것이 아무것도 없을 경우와 Dynamic Object의 프레임 카운팅 처리 
	if (bAutoUpdate == TRUE) {
		S_dwUpdateTime = dwTime;  
...
BOOL __fastcall CMapData::bSetDeadOwner(WORD wObjectID, short sX, short sY, short sType, char cDir, short sAppr1, short sAppr2, short sAppr3, short sAppr4, int iApprColor, short sStatus, char * pName)
{
 int  dX, dY;
 char pTmpName[12];
...
BOOL __fastcall CMapData::bGetOwner(short sX, short sY, char * pName, short * pOwnerType, short * pOwnerStatus, WORD * pObjectID)
{
 int dX, dY;
...
BOOL CMapData::bSetDynamicObject(short sX, short sY, WORD wID, short sType, BOOL bIsEvent)
{
 int dX, dY, sPrevType;
	...
	m_pData[dX][dY].m_cDynamicObjectFrame = rand() % 5;	   // 초기 프레임 값은 랜덤하게 집어 넣는다.
...
void CMapData::GetOwnerStatusByObjectID(WORD wObjectID, char *pOwnerType, char *pDir, short *pAppr1, short *pAppr2, short *pAppr3, short *pAppr4, short * pStatus, int * pColor, char * pName)
{
 int iX, iY;
