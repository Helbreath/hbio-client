    ...
    for (iy = -sModY - ydiff * 32; iy <= get_virtual_height() + (ydiff * 32) * 2; iy += 32)
    {
        ...
        for (ix = -sModX - xdiff * 32; ix <= get_virtual_width() + (xdiff * 32) * 2; ix += 32)
        {
            ...
            if ((ix >= -sModX) && (ix <= get_virtual_width() + 16) && (iy >= -sModY) && (iy <= get_virtual_height() + 32 + 16))
            {
                ...
                _tmp_wObjectID = _tmp_sOwnerType = _tmp_sAppr1 = _tmp_sAppr2 = _tmp_sAppr3 = _tmp_sAppr4 = _tmp_sStatus = 0;
            ...
            if (sObjSpr != 0)
            {
                if ((sObjSpr < 100) || (sObjSpr >= 200))
                {
                    ...
                    switch (sObjSpr)
                    {
                        ...
                        case 429:
                            if (((dwTime - m_dwEnvEffectTime) > 400) && (sObjSprFrame == 2)) bAddNewEffect(65, m_sViewPointX + ix - 15, m_sViewPointY + iy - ((get_virtual_height() / 32) / 2), 0, 0, 0, 0); // 224 | 7
                            break;
                    }
                }
                else
