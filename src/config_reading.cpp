...
void CGame::_LoadTextDlgContents(int cType)
{
    ...
    while (token != NULL)
    {
        m_pMsgTextList[iIndex] = new CMsg(NULL, token, NULL);
        token = pStrTok->pGet();
        iIndex++;
    }
...
int CGame::_iLoadTextDlgContents2(int iType)
{
    ...
    while (token != NULL)
    {
        m_pMsgTextList2[iIndex] = new CMsg(NULL, token, NULL);
        token = pStrTok->pGet();
        iIndex++;
    }
...
void CGame::_LoadGameMsgTextContents()
{
    ...
    while (token != NULL)
    {
        m_pGameMsgList[iIndex] = new CMsg(NULL, token, NULL);
        token = pStrTok->pGet();
        iIndex++;
    }
...
BOOL CGame::bReadItemNameConfigFile()
{
    ...
    hFile = CreateFileA("data\\shops\\ItemName.cfg", GENERIC_READ, NULL, NULL, OPEN_EXISTING, NULL, NULL);
    dwFileSize = GetFileSize(hFile, NULL);
    if (hFile != INVALID_HANDLE_VALUE) CloseHandle(hFile);
...
{
    HANDLE hFile;
    hFile = CreateFileA("CONTENTS\\badword.txt", GENERIC_READ, NULL, NULL, OPEN_EXISTING, NULL, NULL);
    if (hFile == INVALID_HANDLE_VALUE) return FALSE;
    CloseHandle(hFile);

    //	hFile = CreateFileA("SPRITES\\TREES1.PAK", GENERIC_READ, NULL, NULL, OPEN_EXISTING, NULL, NULL);
    //	if( hFile == INVALID_HANDLE_VALUE ) return FALSE;
